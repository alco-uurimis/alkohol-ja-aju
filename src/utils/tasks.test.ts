...
describe('Memory and attention scoring',()=>{
 ...
 it('has eight answerable sourced questions and no fabricated source keys',()=>{expect(questions).toHaveLength(8);expect(new Set(questions.map(q=>q.id)).size).toBe(8);for(const q of questions){expect(q.status).toBe('verified');expect(q.refs.length).toBeGreaterThan(0);for(const id of q.refs)expect(sources.some(s=>s.id===id&&s.url.startsWith('https://'))).toBe(true);}});
});
