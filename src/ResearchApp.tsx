...
export default function ResearchApp(){
  ...
  const download=()=>{
    const blob=new Blob([JSON.stringify({version:3,language:lang,responseId:responseId||undefined,answers},null,2)],{type:'application/json'});
    const url=URL.createObjectURL(blob);
    const a=document.createElement('a');a.href=url;a.download=responseId?`${responseId}.json`:'deidentified-research-response.json';a.click();URL.revokeObjectURL(url);
  };
  ...
  const submit=async()=>{
    ...
    setError('');
    try{
      const response=await fetch(researchEndpoint,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({schemaVersion:3,language:lang,answers})});
  ...
  if(complete)return <div className="research-shell">
    ...
    <main className="research-complete">
      ...
      <p>{ru?'Ответы без прямых идентификаторов сохранены в приватной Google-таблице исследования, а уведомление отправлено организатору в Telegram.':'Otseste tunnusteta vastused salvestati uuringu privaatsesse Google Sheetsi tabelisse ning korraldajale saadeti Telegrami teavitus.'}</p>
  ...
  return <div className="research-shell">
    ...
    <main className="research-main">
      <section className="research-hero">
        <span className="research-kicker">{ru?'ОБЕЗЛИЧЕННЫЙ УЧЕБНО-ИССЛЕДОВАТЕЛЬСКИЙ ОПРОС 18+':'ISIKUSTAMATA ÕPPE- JA UURIMISKÜSITLUS 18+'}</span>
        ...
        <p>{ru?'Около 5–7 минут. Здесь нет правильных ответов — важен реальный опыт и мнение.':'Umbes 5–7 minutit. Õigeid vastuseid ei ole — oluline on tegelik kogemus ja arvamus.'}</p>
        <div className="research-trust"><div><strong>18+</strong><span>{ru?'Опрос предназначен только для совершеннолетних.':'Küsitlus on mõeldud ainult täisealistele.'}</span></div><div><strong>{ru?'Без прямых идентификаторов':'Ilma otseste tunnusteta'}</strong><span>{ru?'Не спрашиваем имя, e-mail, школу, телефон или точную дату рождения.':'Me ei küsi nime, e-posti, kooli, telefoni ega täpset sünnikuupäeva.'}</span></div><div><strong>{ru?'Два места хранения':'Kaks salvestuskohta'}</strong><span>{ru?'После отправки ответы сохраняются в приватной Google Sheet и дублируются сообщением в Telegram организатора.':'Pärast saatmist salvestatakse vastused privaatsesse Google Sheetsi ning koopia saadetakse korraldaja Telegrami.'}</span></div></div>
      </section>
